...
from .layer_pqr import double_layer_pqr, single_layer_pqr
...
def _solver_pqr_neu(
    ...
    L, θ = double_layer_pqr(eparam, k, N)
    A = 0.5 * identity(N) - L
...
def _solver_pqr_pen(obs: Obstacle, k_out: float, trace: Trace, N: int) -> BieSolution:
    """The PQR (Kress) method for the Penetrable Helmholtz problem."""

    k_inn = k_out * sqrt(obs.ρ / obs.σ)

    D_inn, θ = double_layer_pqr(obs.param, k_inn, N)
    D_out, _ = double_layer_pqr(obs.param, k_out, N)
...
def get_total_field_pqr(obs: Obstacle, k: float, trace: Trace, N: int) -> BieSolution:
    """The PQR (Kress) method for the Helmholtz problem."""

    if obs.boundary_type == BoundaryType.NEUMANN:
        return _solver_pqr_neu(obs.param, k, trace, N)

    if obs.boundary_type == BoundaryType.DIRICHLET:
        return _solver_pqr_dir(obs.param, k, trace, N)

    if obs.boundary_type == BoundaryType.PENETRABLE:
        return _solver_pqr_pen(obs, k, trace, N)
